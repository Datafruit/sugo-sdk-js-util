...
var Logger = new (CoreLogger.Logger)('utils').setLevel(CoreLogger.LoggerLevel.error)
...
_.unixTimestamp = function () {
  return Math.round(_.timestamp() / 1000)
}
...
//新的数据包分割格式
_.PlainEncode = (function () {
  return function (mixed_val, _dimensions, serverDimensions) {
    if (!serverDimensions || serverDimensions.length < 1) {
      return null
    }
    // 维度类型：0=long,1=float,2=string;3=dateString;4=date
    var DRUID_COLUMN_TYPE = {
      0: 'l',
      1: 'f',
      2: 's',
      3: 's',
    ...
    var dimensionsObj = serverDimensions.reduce(function (pre, cur) {
      pre[cur.name] = DRUID_COLUMN_TYPE[cur.type]
      return pre
    }, {})
    var value = mixed_val
    var EVENTS_MAPS = { //事件名称映射为中文
      click: '点击',
      change: '改变',
      // submit: '提交',
      focus: '对焦',
    ...
    var HEADER_SPLIT = '\x02' //\002
    var CONTENT_SPLIT = '\x01' //\001
    var dimensions = _dimensions ? _dimensions : Config.dimensions //配置跨域是instance里的
    //s|name,i|age,d|birth\002myName\00125\001672508800000\002yourName\00124\001694926294000
    var formatFn = function (data) {
      var res = [], v, k, item

      for (k in data) {
        if (!_.has(data, k)) continue
        item = data[k]
        if (_.isObject(item) && k === 'properties') {
          v = formatFn(item)
          if (v) {
            res = res.concat(v)
          }
    ...
    var res = keys.join(',') + HEADER_SPLIT + vals.join(CONTENT_SPLIT)
    Logger.log(res)
    return res
  }
})()
...
_.dom_query = (function () {
  ...
  function getElementsBySelector (selector) {
    ...
    var token, bits, tagName, found, foundCount, i, j, k, elements,
      currentContextIndex
...
_.info = {
  ...
  properties: function () {
    ...
    return _.extend(_.strip_empty_properties({
      ...
      'event_time': _.timestamp(),// client date
...
_.compressUrlQuery = function (str) {
  if (_.isObject(str)) {
...
_.decompressUrlQuery = function (str) {
 ...
 * @return {Object}
 */
_.pick = function (object, keyOrKeys) {
  var keys = _.isString(keyOrKeys) ? [keyOrKeys] : keyOrKeys
 ...
 * @return {Object}
 */
_.omit = function (object, keyOrKeys) {
  var oKeys = Object.keys(object)
  var keys = _.isString(keyOrKeys) ? [keyOrKeys] : keyOrKeys
 ...
 * output: true
 *
 * input div#id-selector > a:ntn-child(odd)
 * output: true
 * @param {string} selector
 * @return {boolean}
 */
_.is_nth_child_selector = function (selector) {
